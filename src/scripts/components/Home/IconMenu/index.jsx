...
export default class IconMenu extends React.Component {
  render() {
    ...
    const introIcon = Icon({imageUrl: './icon/center.png', text: '센터 소개', url: '/#/about'})
    const centerIcon = Icon({imageUrl: './icon/camera.png', text: '센터 미리보기', url: '#'})
    const hearingAidIcon = Icon({imageUrl: './icon/hearingaid.png', text: '보청기 소개', url: '#'})
    const blogIcon = Icon({imageUrl: './icon/blog.png', text: '블로그', url: '#'})
    const mapIcon = Icon({imageUrl: './icon/map.png', text: '연락처/위치', url: '#'})
    const storyIcon = Icon({imageUrl: './icon/story.png', text: '사용자 후기', url: '#'})
    return (
      <div>
