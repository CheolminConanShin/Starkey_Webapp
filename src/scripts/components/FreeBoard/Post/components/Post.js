        ...
        return (
            <div className="panel panel-default" style={styles.postBody}>
                <div className="panel-body">
                    <div style={styles.contentField} key={this.props.singlePost.key}>
                        {this.state.content}
                        {this.state.showPasswordForm ?
                            <input className='well-sm' style={Object.assign({}, styles.inputForm, styles.passwordForm)}
                                   value={this.state.password} placeholder="비밀번호"
                                   onChange={this.handlePostEditorPasswordChange}></input> : null}
                        {this.state.showPasswordForm ?
                            <button style={styles.checkButton} onClick={this.checkButtonClick}
                                    className="btn btn-success">답변확인</button> : null}
                        {this.state.showPasswordForm ? null : <div
                                className='well'>{this.state.replyMessage ? this.state.replyMessage : '답변을 준비 중 입니다.'}</div>}
                    </div>
                    {this.state.admin ?
                        <div>
                            <textarea className="form-control" value={this.state.replyMessage}
                                      onChange={this.replyFormChange}/>
                            <button className="btn btn-primary" style={styles.replyButton} onClick={this.handleReply}>
                                답변하기
                            </button>
                        </div> :
                        null
